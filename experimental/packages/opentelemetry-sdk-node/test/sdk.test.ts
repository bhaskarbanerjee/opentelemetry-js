...
describe('Node SDK', () => {
  ...
  describe('configuring metric provider from env', () => {
    ...
    it('should use console with default interval and timeout', async () => {
      ...
      assert(
        sharedState.metricCollectors[0]._metricReader._exportInterval === 60000
      );
      assert(
        sharedState.metricCollectors[0]._metricReader._exportTimeout === 30000
      );
      await sdk.shutdown();
    });
    ...
    it('should use otlp with http/protobuf and and use user defined flushing settings', async () => {
      env.OTEL_METRICS_EXPORTER = 'otlp';
      env.OTEL_EXPORTER_OTLP_METRICS_PROTOCOL = 'http/protobuf';
      env.OTEL_METRICS_PUSH_EXPORTER_INTERVAL = '200';
      env.OTEL_METRICS_PUSH_EXPORTER_TIMEOUT = '150';
      delete env.OTEL_EXPORTER_OTLP_METRICS_PROTOCOL;
      const sdk = new NodeSDK();
      sdk.start();
      const meterProvider = metrics.getMeterProvider();
      ...
      assert(
        sharedState.metricCollectors[0]._metricReader._exportInterval === 200
      );
      assert(
        sharedState.metricCollectors[0]._metricReader._exportTimeout === 150
      );
      await sdk.shutdown();
      delete env.OTEL_METRICS_PUSH_EXPORTER_INTERVAL;
      delete env.OTEL_METRICS_PUSH_EXPORTER_TIMEOUT;
    });
